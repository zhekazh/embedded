...
#include <stdatomic.h> /* _Atomic, atomic_*_explicit, memory_order_* */
...
typedef struct {
    ...
    _Atomic size_t   head;   /* пише лише producer; читає consumer (acquire) */
    _Atomic size_t   tail;   /* пише лише consumer; читає producer (acquire) */
} ringbuf_t;
